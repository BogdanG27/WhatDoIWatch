...
columns_to_select = 'TvShow.Id, TvShow.Name'
...
csv_file_path = 'shows.csv'

# Read the CSV file into a DataFrame
df = pd.read_csv(csv_file_path)


selected_columns = ['title', 'year', 'genre', 'imbd_rating', 'storyline']  # Replace with your actual column names
df = df[selected_columns]

column_mapping = {'title': 'Name', 'storyline': 'Description', 'year': 'ReleaseDate', 'imbd_rating': 'Rating', 'genre': 'Genre'}
...
df['ReleaseDate'] = [datetime(df['ReleaseDate'][i], 1, 1) for i in range(len(df.index))]
...
print(placeholders)
insert_query = f"INSERT INTO  \"TvShow\" ({columns}) VALUES ({placeholders});"
