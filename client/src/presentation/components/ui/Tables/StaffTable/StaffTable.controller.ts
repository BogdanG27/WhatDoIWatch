 ...
 */
export const useStaffTableController = () => {
    const { getStaffs: { key: queryKey, query }, deleteStaff: { key: deleteStaffKey, mutation: deleteStaff } } = useStaffApi();
    ...
    const { mutateAsync: deleteMutation } = useMutation([deleteStaffKey], deleteStaff); // Use a mutation to remove an entry.
