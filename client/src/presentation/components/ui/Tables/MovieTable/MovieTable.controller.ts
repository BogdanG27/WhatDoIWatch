 ...
 */
export const useMovieTableController = () => {
    const { getMovies: { key: queryKey, query }, deleteMovie: { key: deleteMovieKey, mutation: deleteMovie } } = useMovieApi();
    ...
    const { mutateAsync: deleteMutation } = useMutation([deleteMovieKey], deleteMovie); // Use a mutation to remove an entry.
